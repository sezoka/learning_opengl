...
loadModel_assimp :: proc(path: cstring) -> (m: Model) {
    ...
    _processNode_assimp(&meshes, scene.mRootNode, scene);
...
_processNode_assimp :: proc(meshes: ^[dynamic]Mesh, node: ^ass.Node, scene: ^ass.Scene) {
    // process all the node's meshes (if any)
    for i in 0..<node.mNumMeshes {
        mesh := scene.mMeshes[node.mMeshes[i]] 
        append(meshes, _processMesh_assimp(mesh, scene))
    }
    // then do the same for each of its children
    for i in 0..<node.mNumChildren {
        _processNode_assimp(meshes, node.mChildren[i], scene)
    }
}  

_processMesh_assimp :: proc(mesh: ^ass.Mesh, scene: ^ass.Scene) -> Mesh {
